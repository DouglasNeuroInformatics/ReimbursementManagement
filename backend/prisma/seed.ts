...
async function seed() {
  ...
  const admin = await prisma.user.create({
    data: {
      ...
      extension: "201",
      address: "120 Laurier Ave W, Ottawa, ON K1P 5J4",
      updatedAt: now,
    },
  });
  ...
  const admin2 = await prisma.user.create({
    data: {
      ...
      extension: "202",
      address: "1500 Rue de la Montagne, Montréal, QC H3G 1Z3",
      updatedAt: now,
    },
  });
  ...
  const user = await prisma.user.create({
    data: {
      ...
      phone: "+1-403-555-0150",
      extension: "415",
      address: "330 5th Ave SW, Calgary, AB T2P 0L4",
      updatedAt: now,
    },
  });
