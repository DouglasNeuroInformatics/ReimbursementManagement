...
function NewTravelAdvancePage() {
  ...
  const handleSave = async (andSubmit = false) => {
    ...
    const { itemErrors: fieldErrs } = validateExpenseItems(
      items.map((it) => ({ label: it.category, amount: it.amount, date: '' })),
    ...
    if (Object.keys(te).length > 0 || Object.keys(fieldErrs).length > 0) {
      const targetId = te.title ? 'ta-title' : te.destination ? 'ta-destination' : te.purpose ? 'ta-purpose' : `ta-item-${Object.keys(fieldErrs)[0]}`
    ...
    <div className="max-w-2xl space-y-5">
      ...
      <Card><CardHeader><span className="font-semibold">{t('details')}</span></CardHeader>
        <CardBody className="space-y-4">
          <div id="ta-title"><Input label={t('fields.title') as string} value={form.title} onChange={set('title')} error={topErrors.title ? (t(topErrors.title) as string) : undefined} required /></div>
      ...
      <Card><CardHeader><span className="font-semibold">{t('tripDetails')}</span></CardHeader>
        <CardBody className="space-y-4">
          <div id="ta-destination"><Input label={t('fields.destination') as string} value={form.destination} onChange={set('destination')} error={topErrors.destination ? (t(topErrors.destination) as string) : undefined} required /></div>
          <div id="ta-purpose"><Input label={t('fields.purpose') as string} value={form.purpose} onChange={set('purpose')} error={topErrors.purpose ? (t(topErrors.purpose) as string) : undefined} required /></div>
        ...
        <CardBody>
          <div className="space-y-2">
            {items.map((it, i) => {
              ...
              return (
              <div key={it._key} id={`ta-item-${i}`} className="grid grid-cols-3 gap-2 items-start">
