  ...
  return (
    <aside className="w-64 glass-dark !bg-white/40 border border-white/60 flex flex-col shrink-0 rounded-2xl shadow-sm overflow-hidden hidden md:flex animate-slide-in">
      ...
      <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
        ...
        <NavItem to="/dashboard/requests">{t('nav.myRequests')}</NavItem>
        {(user.role === 'SUPERVISOR' || user.role === 'FINANCIAL_ADMIN') && (
          <div className="pt-4 pb-1">
            <span className="px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">{t('nav.management')}</span>
          </div>
        )}
        {(user.role === 'SUPERVISOR' || user.role === 'FINANCIAL_ADMIN') && (
          <>
            <NavItem to="/review">{t('nav.reviewQueue')}</NavItem>
            <NavItem to="/review/history">{t('nav.requestHistory')}</NavItem>
          </>
        )}
        {user.role === 'FINANCIAL_ADMIN' && (
          <div className="pt-4 pb-1">
            <span className="px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">{t('nav.administration')}</span>
          </div>
        )}
        {user.role === 'FINANCIAL_ADMIN' && (
          <>
      ...
      <div className="p-4 border-t border-white/40 bg-white/20">
        ...
        >
          ...
          <div className="opacity-70 font-medium">{t(`role.${user.role}`, { ns: 'enums' }) as string}</div>
        </Link>
      </div>
    </aside>
  )
